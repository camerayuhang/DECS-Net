...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.embedding(x)
        x = self.norm(x)
        x = x.reshape(B, self.hidden_dim, (H // self.ps) * (W // self.ps)).permute(0, 2, 1)  # x become linear embeding
        # x = x + self.pos(x).to(x)
        return x
...
class FCHiLo1(nn.Module):
    ...
    def hi_lofi(self, x):  # this code is lying to the reader, first they use windows size of 8 to downsample the feature map directly, rather than using WT to downsample; Second, they do not calculate the self-attention within the windows, which means windows_size is a useless parameter
        B, N, C = x.shape
        H = W = int(N ** 0.5)
        x = x.reshape(B, H, W, C).permute(0, 3, 1, 2)
        ...
        high_feats = high_feats - x
        # qkv here is obtained by convolution operartion instead of matrix multiplication
        if self.l_heads != 0:  # l_q [B, heads, H*W, C]
            l_q = self.l_q(x).permute(0, 2, 3, 1).reshape(B, H * W, self.l_heads, self.l_dim // self.l_heads).permute(0, 2, 1, 3)  # assigned channels to different head using dsc. i.e l_q
            if self.ws > 1:  # l_kv [2, B, heads, H*W, C],
            ...
            l_attn = (l_q @ l_k.transpose(-2, -1)) * self.scale  # must divided by sqrt of head dim when calculating self-attention, which is self.scale in this code
            l_attn = l_attn.softmax(dim=-1)

            l_x = (l_attn @ l_v).transpose(1, 2).reshape(B, H, W, self.l_dim).permute(0, 3, 1, 2)  # concatnate all channels from each head
            l_x = self.l_proj(l_x).permute(0, 2, 3, 1)  # reshape back to [B, H, W, C] for fusing with feature map from CNN

        if self.h_heads != 0:  # It seems
            h_group, w_group = H // self.ws, W // self.ws
            total_groups = h_group * w_group  # h_qkv [3, B, windows_num, heads, windows_size**2, C]
...
class Backbone(nn.Module):
    ...
    def forward(self, x):
        x = self.embedding(x)  # they use dw with same size of k_size and padding to perform patch partition and linear embeding, result in feature map with 4x downsampling of origin image
        x1 = self.block1(x)
        x1 = self.block12(x1)
        x1 = self.block13(x1)

        x2 = self.merge1(x1)  # 2x downsampling
        x2 = self.block2(x2)
        x2 = self.block22(x2)
        x2 = self.block23(x2)

        x3 = self.merge2(x2)  # 2x downsampling
        x3 = self.block3(x3)
        x3 = self.block32(x3)
        x3 = self.block33(x3)

        x4 = self.merge3(x3)  # 2x downsampling
        out = self.block4(x4)
